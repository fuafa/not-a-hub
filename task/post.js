...
fs.readFile(postTemplatePath, 'utf8', (err, data) => Promise.all(
	files.map(path => new Promise((resolve, reject) => fs.readFile(
    `${sourcePostsPath}/${path}`, 'utf8', (err, md) => {
    if (err) {
   ...
   */
  fs.writeFile(`${outPostsPath}/PostList.json`, JSON.stringify(posts, null, 2), err => {
    if (err) {
