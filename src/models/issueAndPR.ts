...
export type IssueOrPRPropsMata = typeof Fake;
export type IssueOrPRProps = IssueOrPRPropsMata['items'][number];

const octokit = new Octokit();

const issuesAndPRs = createModel({
  state: Fake,
  reducers: {
    setFeeds(state: any, payload: any) {
      return payload;
    }
  },
  effects: (dispatch) => ({
    async getFeeds(payload?: 'issue' | 'pr') {
      const q = `author:fuafa${payload ? '+is:' + payload : ''}`;
      console.log(q);
      const feeds = await octokit.search.issuesAndPullRequests({
        q
      });
      console.log(feeds);
      dispatch.issuesAndPRs.setFeeds(feeds.data);
    }
  }
)});

export default issuesAndPRs;